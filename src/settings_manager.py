...
class SettingsManager:
    ...
    def __init__(self, app_state: AppState):
        ...
        logger.debug(
            f"SettingsManager initialized. File path: {self.settings_file_path}"
        )
    ...
    def load_settings(self) -> None:
        ...
        try:
            if self.settings_file_path.is_file():
                ...
                logger.info(
                    f"Settings loaded successfully from {self.settings_file_path}"
                )
            else:
                logger.info(
                    f"Settings file {self.settings_file_path} not found, using defaults."
                )
        except (OSError, json.JSONDecodeError) as e:
            logger.warning(
                f"Could not load or parse settings from {self.settings_file_path}: {e}"
            )
        ...
        logger.debug(
            f"Initial settings captured after load: {self.initial_settings_dict}"
        )
    ...
    def save_settings(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Could not save settings to {self.settings_file_path}: {e}",
                exc_info=True,
            )
            return False
