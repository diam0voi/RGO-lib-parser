...
ShowMessageCallback = Callable[
    [str, str, str], None
]  # type (info/warn/error), title, message
...
class TaskManager:
    ...
    def _start_thread(self, target_func: callable, args: tuple, task_name: str):
        ...
        if self.is_running():
            logger.warning(
                f"Task '{task_name}' requested, but another task is already running."
            )
    ...
    def _thread_wrapper(
        self, target_func: callable, *args, task_name: str = "Task", **kwargs
    ) -> None:
        ...
        try:
            ...
            if target_func == self.handler.download_pages:
                ...
                if not self.stop_event.is_set():
                    if success_count == total_dl_pages and total_dl_pages > 0:
                        ...
                        self.show_message_cb(
                            ...
                            f"Скачано {success_count} из {total_dl_pages} страниц.\nПроверьте лог.",
                        )
                ...
                final_message = (
                    f"Обработка завершена. Обработано/скопировано: {processed_count}."
                )
                ...
                if not self.stop_event.is_set():  # pragma: no branch
                    ...
                    self.show_message_cb(
                        "info",
                        "Успех",
                        f"Создание разворотов завершено!\n{final_message}",
                    )
            ...
            logger.critical(
                f"Critical error in thread ({task_name}): {e}", exc_info=True
            )
            self.show_message_cb(
                "error",
                "Критическая ошибка",
            ...
            if not self.stop_event.is_set():
                ...
                if folder_to_open and success and self.root:
                    ...
                    self.root.after(
                        500, lambda p=folder_to_open: self.open_folder_cb(p)
                    )
            ...
            logger.info(
                f"Thread finished for: {task_name}. Success: {success}, Interrupted: {self.stop_event.is_set()}, Error: {error_occurred}"
            )
    ...
    def _run_all_sequence(
        ...
        try:
            ...
            if download_success_count == 0:
                self.status_cb(
                    "--- Скачивание не удалось (0 страниц), обработка пропущена ---"
                )
                ...
                self.status_cb(
                    f"--- Скачивание успешно завершено ({success_count}/{total_dl_pages}) ---"
                )
            ...
            logger.critical(
                f"Critical error in combined sequence ({task_name}): {e}", exc_info=True
            )
