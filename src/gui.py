...
from . import ui_builder # Импортируем модуль целиком
...
class JournalDownloaderApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.widgets.update(ui_builder.create_input_frame(
            self.root, self.state, self.browse_output_pages, self.browse_output_spreads
        ))
        self.widgets.update(ui_builder.create_control_frame(
            self.root, self.run_all, self.run_download, self.run_processing, self.stop_action
        ))
    ...
    def browse_output_pages(self) -> None:
        ...
        directory = filedialog.askdirectory(initialdir=initial_dir, title="Выберите папку для скачивания страниц", parent=self.root)
        if directory:
    ...
    def browse_output_spreads(self) -> None:
        ...
        if not os.path.isdir(initial_dir):
            initial_dir = str(Path.home())
        directory = filedialog.askdirectory(initialdir=initial_dir, title="Выберите папку для сохранения разворотов", parent=self.root)
        if directory:
    ...
    def _update_status(self, message: str) -> None:
        ...
        if not isinstance(status_text_widget, scrolledtext.ScrolledText): return
    ...
    def _update_progress(self, current_value: int, max_value: int) -> None:
        ...
        if not isinstance(progress_bar_widget, ttk.Progressbar): return
    ...
    def _set_buttons_state(self, task_running: bool) -> None:
        ...
        try:
            ...
            for key in ['download_button', 'process_button', 'run_all_button', 'browse_pages_button', 'browse_spreads_button']:
                button = self.widgets.get(key)
                if button: button.config(state=action_buttons_state)

            stop_button = self.widgets.get('stop_button')
            if stop_button: stop_button.config(state=stop_button_state)
    ...
    def _show_message_safe(self, msg_type: str, title: str, message: str) -> None:
        ...
        if self.root and self.root.winfo_exists():
            self.root.after(0, self._show_message, msg_type, title, message)
             ...
             logger.warning(f"GUI root doesn't exist, message ignored: [{msg_type}] {title} - {message}")
    ...
    def _open_folder_safe(self, folder_path: str) -> None:
        ...
        if self.root and self.root.winfo_exists():
            ...
            logger.warning(f"GUI root doesn't exist, open folder ignored: {folder_path}")
    ...
    def _open_folder(self, folder_path: str) -> None:
        ...
        try:
            ...
            if os.path.isdir(norm_path):
                os.startfile(norm_path) # Должно работать на Windows, MacOS, Linux (с xdg-open)
                ...
                self._show_message("error", "Ошибка", f"Не удалось открыть папку, так как она не найдена:\n{norm_path}")
        except Exception as e:
            err_msg = f"Ошибка при открытии папки '{folder_path}': {e}"
            self._update_status(err_msg)
            logger.error(err_msg, exc_info=True)
            self._show_message("error", "Ошибка", f"Произошла ошибка при попытке открыть папку:\n{e}")
    ...
    def _validate_and_show_errors(self, validation_func: Callable[..., list[str]], *args, **kwargs) -> bool:
    ...
    def run_download(self) -> None:
        ...
        if not self.settings_manager.save_settings():
             self._show_message('warning', "Ошибка сохранения", "Не удалось сохранить настройки перед запуском.")
    ...
    def run_all(self) -> None:
        ...
        if not self._validate_and_show_errors(self.state.validate_for_download):
            return
        if not self._validate_and_show_errors(self.state.validate_for_processing, check_dir_exists=False):
             return
        if not self.settings_manager.save_settings():
             self._show_message('warning', "Ошибка сохранения", "Не удалось сохранить настройки перед запуском.")
        self.clear_status()
