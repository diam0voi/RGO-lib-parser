...
def create_input_frame(
    parent: tk.Widget,
    app_state: AppState,
    browse_pages_cmd: Callable,
    browse_spreads_cmd: Callable,
) -> WidgetsDict:
    ...
    ttk.Label(input_frame, text="Базовый URL (до ID):").grid(
        row=0, column=0, sticky=tk.W
    )
    widgets["url_base_entry"] = ttk.Entry(
        input_frame, width=60, textvariable=app_state.url_base
    )
    widgets["url_base_entry"].grid(row=0, column=1, columnspan=2, sticky=tk.EW)

    ttk.Label(input_frame, text="ID файла (часть URL):").grid(
        row=1, column=0, sticky=tk.W
    )
    widgets["url_ids_entry"] = ttk.Entry(
        input_frame, width=60, textvariable=app_state.url_ids
    )
    widgets["url_ids_entry"].grid(row=1, column=1, columnspan=2, sticky=tk.EW)

    ttk.Label(input_frame, text="Имя файла на сайте:").grid(
        row=2, column=0, sticky=tk.W
    )
    widgets["pdf_filename_entry"] = ttk.Entry(
        input_frame, width=60, textvariable=app_state.pdf_filename
    )
    ...
    widgets["total_pages_entry"] = ttk.Entry(
        input_frame, width=10, textvariable=app_state.total_pages
    )
    ...
    widgets["pages_dir_entry"] = ttk.Entry(
        path_frame, width=45, textvariable=app_state.pages_dir
    )
    widgets["pages_dir_entry"].grid(row=0, column=1, sticky=tk.EW, padx=(0, 5))
    widgets["browse_pages_button"] = ttk.Button(
        path_frame, text="Обзор...", command=browse_pages_cmd
    )
    widgets["browse_pages_button"].grid(row=0, column=2)

    ttk.Label(path_frame, text="Папка для разворотов:").grid(
        row=1, column=0, sticky=tk.W
    )
    widgets["spreads_dir_entry"] = ttk.Entry(
        path_frame, width=45, textvariable=app_state.spreads_dir
    )
    widgets["spreads_dir_entry"].grid(row=1, column=1, sticky=tk.EW, padx=(0, 5))
    widgets["browse_spreads_button"] = ttk.Button(
        path_frame, text="Обзор...", command=browse_spreads_cmd
    )
...
def create_control_frame(
    parent: tk.Widget,
    run_all_cmd: Callable,
    download_cmd: Callable,
    process_cmd: Callable,
    stop_cmd: Callable,
) -> WidgetsDict:
    ...
    widgets["run_all_button"] = ttk.Button(
        control_frame, text="Скачать и создать развороты", command=run_all_cmd
    )
    widgets["run_all_button"].pack(side=tk.LEFT, padx=5)
    widgets["download_button"] = ttk.Button(
        control_frame, text="Только скачать страницы", command=download_cmd
    )
    widgets["download_button"].pack(side=tk.LEFT, padx=5)
    widgets["process_button"] = ttk.Button(
        control_frame, text="Только создать развороты", command=process_cmd
    )
    widgets["process_button"].pack(side=tk.LEFT, padx=5)
    widgets["stop_button"] = ttk.Button(
        control_frame,
        text="СТОП",
        command=stop_cmd,
        state=tk.DISABLED,
...
def create_status_frame(parent: tk.Widget) -> WidgetsDict:
    ...
    widgets["status_text"] = scrolledtext.ScrolledText(
        status_frame, height=10, wrap=tk.WORD, state=tk.DISABLED, bd=0, relief=tk.FLAT
    )
