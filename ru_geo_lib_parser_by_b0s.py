...
class JournalDownloaderApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Загрузчик + склейщик файлов из библиотеки РГО. v1.2 by b0s")
        ...
        self.pages_dir_entry.insert(0, "C:/downloaded_pages")
        ...
        self.spreads_dir_entry.insert(0, "C:/final_spreads")
    ...
    def _validate_download_inputs(self):
        if not self.url_base_entry.get() or not self.url_ids_entry.get() or \
           ...
           not self.pages_dir_entry.get():
            messagebox.showerror("Ошибка ввода", "Пожалуйста, заполните все поля URL, имени, кол-ва страниц и папки для страниц.")
            return False
        try:
    ...
    def _thread_download(self):
        ...
        try:
            ...
            try:
                ...
                self.root.after(0, lambda: messagebox.showerror("Ошибка папки", f"Не удалось создать папку:\n{output_dir}\n{e}"))
                return

            session = requests.Session()
            session.headers.update(headers)

            self.root.after(0, lambda: self.update_status(f"Автоматическое получение сессионных куки с {INITIAL_COOKIE_URL}..."))
            try:
                initial_response = session.get(INITIAL_COOKIE_URL, timeout=20)
                initial_response.raise_for_status() # Проверяем на ошибки HTTP (4xx, 5xx)

                if session.cookies:
                    cookie_names = list(session.cookies.keys())
                    self.root.after(0, lambda cn=cookie_names: self.update_status(f"Успешно получены куки: {cn}"))
                else:
                    self.root.after(0, lambda: self.update_status("Предупреждение: Не удалось автоматически получить куки (сервер не установил?). Попытка продолжить без них..."))

            except requests.exceptions.Timeout:
                 self.root.after(0, lambda: self.update_status(f"Ошибка: Превышено время ожидания при получении куки с {INITIAL_COOKIE_URL}."))
                 self.root.after(0, lambda: messagebox.showwarning("Ошибка сети", f"Не удалось подключиться к {INITIAL_COOKIE_URL} для получения куки (таймаут). Загрузка может не работать."))
            except requests.exceptions.RequestException as e:
                 self.root.after(0, lambda: self.update_status(f"Ошибка при получении куки: {e}. Загрузка может не работать."))
                 self.root.after(0, lambda: messagebox.showwarning("Ошибка сети", f"Ошибка при получении куки: {e}. Загрузка может не работать."))
            ...
            for i in range(total_pages):
                ...
                try:
                    ...
                    if os.path.getsize(final_output_filename) == 0:
                     ...
                     error_msg = f"Ошибка HTTP {e.response.status_code} на стр. {i+1}: {e}"
                     self.root.after(0, lambda msg=error_msg: self.update_status(msg))
                     if e.response.status_code in [401, 403]:
                         self.root.after(0, lambda: self.update_status("   (Возможно, сессия истекла или куки неверны)"))
                except requests.exceptions.RequestException as e:
            ...
            if not self.stop_event.is_set():
                ...
                should_open = success_count > 0
                if should_open:
                    self.root.after(500, lambda path=folder_to_open: self.open_folder(path))
                ...
                else: # Ничего не скачано
                     self.root.after(0, lambda: messagebox.showerror("Ошибка", f"Не удалось скачать ни одной страницы. Проверьте URL, ID, имя файла и лог ошибок."))


        except Exception as e:
            error_msg = f"Критическая ошибка при скачивании: {e}"
            traceback_str = traceback.format_exc()
            print(f"CRITICAL DOWNLOAD ERROR:\n{traceback_str}")
            self.root.after(0, lambda msg=error_msg: self.update_status(msg))
            self.root.after(0, lambda: messagebox.showerror("Критическая ошибка", f"Произошла критическая ошибка при скачивании:\n{e}\n\n(Подробности в консоли)"))
        finally:
            self.stop_event.clear()
            self.current_thread = None
    ...
    def _thread_process(self):
        ...
        try:
            ...
            while page_index < len(sorted_files):
                ...
                # Вариант 1: Копирование обложки (первая страница) или готового разворота
                ...
                # --- Вариант 2: Текущий файл - одиночная страница (не первая и не разворот) ---
                    ...
                    if next_is_single:
                        ...
                        try:
                            with Image.open(current_path) as img_left, Image.open(next_path) as img_right:
                                ...
                                # Приводим к средней высоте, если разные
                                if h_left != h_right:
                                    target_height = (h_left + h_right) // 2
                                    self.root.after(0, lambda h=target_height: self.update_status(f"   (Выравниваю высоту до {h}px)"))
                                ...
                                # Создаем новое изображение в RGB (для сохранения в JPG)
                                spread_img = Image.new('RGB', (total_width, target_height), (255, 255, 255)) # Белый фон

                                spread_img.paste(img_left_final.convert('RGB'), (0, 0))
                                spread_img.paste(img_right_final.convert('RGB'), (w_left_final, 0))

                                spread_img.save(output_path, "JPEG", quality=95, optimize=True)
                                created_spread_count += 1
                                processed_count += 2 
                        except Exception as e:
                            error_msg = f"Ошибка при создании разворота для {current_file} и {next_file}: {e}"
                            self.root.after(0, lambda msg=error_msg: self.update_status(msg))
                        page_index += 2 # Пропускаем пару
                        continue

                # Вариант 2.2: Текущая одиночная страница, но следующая - разворот или последняя
                _, ext = os.path.splitext(current_file)
                output_filename = f"spread_{current_page_num:03d}{ext}"
                output_path = os.path.join(output_folder, output_filename)
            ...
            if not self.stop_event.is_set():
                ...
                if created_spread_count > 0:
                    final_msg += f" Создано {created_spread_count} разворотов."
                self.root.after(0, lambda msg=final_msg: self.update_status(msg))
            ...
            import traceback
            traceback_str = traceback.format_exc()
            print(f"CRITICAL PROCESSING ERROR:\n{traceback_str}")
            self.root.after(0, lambda msg=error_msg: self.update_status(msg))
            self.root.after(0, lambda: messagebox.showerror("Критическая ошибка", f"Произошла критическая ошибка при обработке:\n{e}\n\n(Подробности в консоли)"))
        finally:
            self.stop_event.clear()
            self.current_thread = None
            self.root.after(0, lambda: self._set_buttons_state(task_running=False))
    ...
    def _thread_run_all(self):
        ...
        # Проверяем результат скачивания перед запуском обработки
        input_folder = self.pages_dir_entry.get().strip()
        try:
            # Существует ли папка и есть ли в ней файлы
            if not os.path.isdir(input_folder) or not os.listdir(input_folder):
             ...
             self.root.after(0, lambda: self.update_status(f"Ошибка проверки папки '{input_folder}': {e}. Пропуск обработки."))
             self.root.after(0, lambda: messagebox.showerror("Ошибка папки", f"Не удалось проверить папку '{input_folder}': {e}"))
             self.root.after(0, lambda: self._set_buttons_state(task_running=False))
             return
