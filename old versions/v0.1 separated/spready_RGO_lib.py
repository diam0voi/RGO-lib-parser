...
SPREAD_ASPECT_RATIO_THRESHOLD = 1.1  # Порог соотношения сторон (ширина / высота) для определения разворота.
...
def is_likely_spread(image_path, threshold):
    try:
        ...
        print(f"Warning: Could not read image {os.path.basename(image_path)} to check aspect ratio: {e}")
        return False
...
def create_smart_spreads(input_folder, output_folder):
    ...
    print(f"Найдено {len(sorted_files)} страниц/файлов. Начинаем умное создание разворотов...")
    ...
    while page_index < len(sorted_files):
        ...
        current_is_spread = page_index > 0 and is_likely_spread(current_path, SPREAD_ASPECT_RATIO_THRESHOLD)
        ...
        if current_is_spread or page_index == 0:
            ...
            try:
                print(f"Копирую {'обложку' if page_index == 0 else 'готовый разворот'}: {current_file} -> {output_filename}")
        ...
        if page_index + 1 < len(sorted_files):  # Проверяем, есть ли следующий файл
            ...
            next_is_single = not is_likely_spread(next_path, SPREAD_ASPECT_RATIO_THRESHOLD)

            # Вариант 2.1: Следующий файл тоже одиночный - СКЛЕИВАЕМ 
            if next_is_single:
                output_filename = f"spread_{current_page_num:03d}-{next_page_num:03d}.jpg"
                output_path = os.path.join(output_folder, output_filename)
                print(f"Создаю разворот из одиночных: {current_file} + {next_file} -> {output_filename}")
                ...
                try:
                    ...
                    spread_img = Image.new('RGB', (total_width, max_height), SPREAD_BG_COLOR)
                    ...
                    print(f"Ошибка при создании разворота для {current_file} и {next_file}: {e}")
        ...
        output_path = os.path.join(output_folder, output_filename)
        try:
            print(f"Копирую одиночную страницу (нет пары или след. - разворот): {current_file} -> {output_filename}")
            img = Image.open(current_path)
            img.convert('RGB').save(output_path, "JPEG", quality=95)
            img.close()
        except Exception as e:
