...
class TestLibraryHandler:
    ...
    def test_download_pages_success(self, library_handler, mock_session, mock_callbacks, mock_path, mocker):
        ...
        mock_callbacks["status_callback"].assert_any_call(f"Скачивание завершено. Успешно: {total_pages} из {total_pages}.")
    ...
    def test_download_pages_stop_event(self, library_handler, mock_session, mock_callbacks, mock_path, mocker):
        ...
        mock_path.return_value.stat.side_effect = None # Убедимся, что нет лишних side_effect
        ...
        mock_callbacks["progress_callback"].assert_called_with(stop_at_page, total_pages)
        ...
        assert mock_path.return_value.stat.call_count == stop_at_page # Вызван для i=0, i=1
    ...
    def test_download_pages_io_error(self, library_handler, mock_session, mock_callbacks, mock_path, mocker):
        ...
        mock_file_open.return_value.write.side_effect = IOError(error_msg)
        ...
        logic.logger.error.assert_called_with(f"Ошибка записи файла для стр. 1: {error_msg} Filename: {expected_path_object}", exc_info=True)
    ...
    def test_download_pages_content_types(self, library_handler, mock_session, mock_callbacks, mock_path, mocker, content_type, expected_suffix):
        ...
        is_unknown_type = expected_suffix == ".jpg" and 'jpeg' not in main_content_type and 'jpg' not in main_content_type
