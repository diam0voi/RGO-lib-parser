...
from unittest.mock import MagicMock  # Используем MagicMock для мока AppState
...
def mock_app_state(mocker) -> MagicMock:
    ...
    mock = mocker.MagicMock(
        name="AppStateMock"
    )  # Дадим имя для ясности в логах/ошибках
...
def settings_manager(
    mock_app_state: MagicMock, temp_settings_file: Path, mocker
) -> SettingsManager:
...
def test_load_settings_file_not_found(
    settings_manager: SettingsManager, mock_app_state: MagicMock, caplog
):
    ...
    # Проверяем логи
    assert (
        f"Settings file {settings_manager.settings_file_path} not found, using defaults."
        in caplog.text
    )


def test_load_settings_file_exists_valid_json(
    settings_manager: SettingsManager,
    mock_app_state: MagicMock,
    temp_settings_file: Path,
    caplog,
    ...
    temp_settings_file.parent.mkdir(
...
def test_load_settings_file_exists_invalid_json(
    settings_manager: SettingsManager,
    mock_app_state: MagicMock,
    temp_settings_file: Path,
    caplog,
...
def test_save_settings_success(
    ...
    mock_app_state._current_state = (
        settings_to_save.copy()
    )  # Устанавливаем через имитируемое состояние
...
def test_save_settings_unexpected_error(
    settings_manager: SettingsManager,
    mock_app_state: MagicMock,
    temp_settings_file: Path,
    mocker,
    ...
    mocker.patch(
        "src.settings_manager.json.dump", side_effect=TypeError("Unexpected type")
    )
...
def test_save_settings_if_changed_when_changed(
    settings_manager: SettingsManager, mock_app_state: MagicMock, mocker, caplog
):
...
def test_save_settings_if_changed_when_unchanged(
    settings_manager: SettingsManager, mock_app_state: MagicMock, mocker, caplog
):
...
def test_save_settings_if_changed_when_changed_but_save_fails(
    settings_manager: SettingsManager, mock_app_state: MagicMock, mocker, caplog
):
