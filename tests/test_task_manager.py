...
def mock_deps(mocker):
    ...
    def capture_target(*args, **kwargs):
        ...
        mock_thread_instance.start.side_effect = lambda: print(
            f"Mock Thread started for {target.__name__ if hasattr(target, '__name__') else repr(target)}"
        )
    ...
    patcher_config_log = mocker.patch(
        "src.task_manager.config.LOG_FILE", EXPECTED_MOCKED_LOG_FILE
    )
...
def test_start_processing_success(task_manager, mock_deps):
    ...
    final_msg_from_code = (
        "Обработка завершена. Обработано/скопировано: 10. Создано разворотов: 5."
    )
    mock_deps["show_message_cb"].assert_called_once_with(
...
def test_thread_wrapper_exception(task_manager, mock_deps):
    ...
    expected_error_msg_detail = (
        f"{task_name}: Критическая ошибка при выполнении: {error_message}"
    )
    # FIX: Используем константу EXPECTED_MOCKED_LOG_FILE
    mock_deps["show_message_cb"].assert_called_once_with(
        "error",
    ...
    mock_deps["status_cb"].assert_called_once_with(
        f"--- {expected_error_msg_detail} ---"
    )
    mock_deps["open_folder_cb"].assert_not_called()
    mock_deps["set_buttons_state_cb"].assert_has_calls([call(True), call(False)])
...
def test_stop_task_when_running(task_manager, mock_deps):
    ...
    mock_deps["status_cb"].assert_called_once_with(
        "--- Получен сигнал СТОП от пользователя ---"
    )
...
def test_start_all_full_success(task_manager, mock_deps, setup_start_all):
    ...
    final_msg_sequence = (
        "Скачивание (10/10) и обработка (10 файлов, 5 разворотов) завершены."
    )
    ...
    mock_deps["show_message_cb"].assert_called_once_with(
        "info", "Завершено", final_msg_sequence
    )
    mock_deps["open_folder_cb"].assert_called_once_with("/path/to/spreads")
    mock_deps["set_buttons_state_cb"].assert_has_calls([call(True), call(False)])
    assert task_manager.current_thread is None
...
def test_start_all_download_partial_then_process(
    task_manager, mock_deps, setup_start_all
):
    ...
    final_msg_sequence = (
        "Скачивание (7/10) и обработка (7 файлов, 4 разворотов) завершены."
    )
    final_msg_wrapper = f"{task_name}: Задача завершена."
    mock_deps["status_cb"].assert_has_calls(
        [
            call("--- НАЧАЛО: Скачивание страниц ---"),
            call(
                "--- Скачивание завершено с ошибками (7/10). Продолжаем обработку скачанных... ---"
            ),
            call("--- НАЧАЛО: Создание разворотов ---"),
            call(f"--- {final_msg_sequence} ---"),
            call(f"--- {final_msg_wrapper} ---"),
...
def test_start_all_stopped_during_download(task_manager, mock_deps, setup_start_all):
    ...
    assert (
        call(f"--- {final_msg_wrapper} ---")
        not in mock_deps["status_cb"].call_args_list
    )
    mock_deps["show_message_cb"].assert_not_called()
    mock_deps["open_folder_cb"].assert_not_called()
    mock_deps["set_buttons_state_cb"].assert_has_calls([call(True), call(False)])
...
def test_start_all_exception_during_processing(
    task_manager, mock_deps, setup_start_all
):
    ...
    mock_deps["status_cb"].assert_has_calls(
        [
            ...
            call(
                f"--- {expected_error_msg_wrapper} ---"
            ),  # Финальное из wrapper с ошибкой
        ]
    )
...
def test_start_all_download_ok_process_zero(task_manager, mock_deps, setup_start_all):
    ...
    )  # Выполняем обертку с _run_all_sequence
    ...
    final_msg_sequence = (
        "Скачивание (10/10) и обработка (0 файлов, 0 разворотов) завершены."
    )
    final_msg_wrapper = f"{task_name}: Задача завершена."
    ...
    mock_deps["status_cb"].assert_has_calls(
        [
            ...
            call(
                f"--- {final_msg_wrapper} ---"
            ),  # Сообщение из _thread_wrapper.finally
        ],
        any_order=False,
    )
...
def test_start_all_without_root(task_manager_no_root, mock_deps):
    ...
    assert (
        captured_args[0] == tm._run_all_sequence
    )  # Первый аргумент обертки - наша целевая функция
    wrapper_target = mock_deps["thread_targets"]["target"]  # Это _thread_wrapper
    wrapper_args = (
        tm._run_all_sequence,
        *expected_args,
    )  # Аргументы для _thread_wrapper
    ...
    mock_deps[
        "open_folder_cb"
    ].assert_not_called()  # Как следствие, и колбэк не вызвался
...
def test_thread_wrapper_stop_event_is_set_for_checks(task_manager, mock_deps):
    ...
    mock_deps["handler"].download_pages.return_value = (
        10,
        10,
    )  # Результат не важен, т.к. блок обработки пропустим
    ...
    captured_args = mock_deps["thread_targets"][
        "args"
    ]  # (handler.download_pages, *expected_args)
    ...
    mock_deps[
        "open_folder_cb"
    ].assert_not_called()  # Проверяем, что папка не открывалась вообще
        ...
        for after_call in task_manager.root.after.call_args_list:
            ...
            if delay == 0:  # Ищем вызов с нулевой задержкой
                ...
                try:
                    ...
                    task_manager.set_buttons_state_cb = (
                        original_set_buttons  # Восстанавливаем
                    )
        assert found_button_reset_call, (
            "root.after(0, ...) для set_buttons_state_cb(False) не был вызван"
        )
