...
from unittest.mock import (
    ...
    MagicMock,
    call,
)  # ANY поможет проверять вызовы с динамическими аргументами

from PIL import Image  # Нужен для моков и проверки типов
import pytest
...
def create_mock_files(mocker, file_paths):
    ...
    mock_input_path.__str__.return_value = (
        str(file_paths[0].parent) if file_paths else "mock_input_path"
    )
...
def test_process_images_success_copy_and_merge(
    ...
    # Создаем моки файлов
    mock_input_path, mock_iterdir_results = create_mock_files(
        mocker, mock_file_paths_real
    )
    ...
    mock_image_open = mocker.patch(
        "src.image_processing.Image.open", return_value=mock_pil_image
    )
    mock_image_new = mocker.patch(
        "src.image_processing.Image.new", return_value=mock_pil_image
    )  # Image.new тоже возвращает Image
    ...
    def image_open_side_effect(filepath_arg):
        ...
        if "001" in filepath_str:
            return mock_img1
        if "002" in filepath_str:
            return mock_img2
    ...
    assert (
        result == (4, 1)
    )  # 1 копия (обложка) + 2 склеены + 1 копия (последняя) = 4 обработано, 1 разворот создан
    ...
    mock_utils.is_likely_spread.assert_any_call(
        mock_iterdir_results[1], mock_config.DEFAULT_ASPECT_RATIO_THRESHOLD
    )  # 001_page.png
    mock_utils.is_likely_spread.assert_any_call(
        mock_iterdir_results[2], mock_config.DEFAULT_ASPECT_RATIO_THRESHOLD
    )  # 002_page.jpg (проверяется как next)
    mock_utils.is_likely_spread.assert_any_call(
        mock_iterdir_results[3], mock_config.DEFAULT_ASPECT_RATIO_THRESHOLD
    )  # 003_single.jpeg
    ...
    w2_final = 810
    mock_img1.resize.assert_called_with(
        (w1_final, target_height), Image.Resampling.LANCZOS
    )
    mock_img2.resize.assert_called_with(
        (w2_final, target_height), Image.Resampling.LANCZOS
    )

    mock_image_new.assert_called_once_with(
        "RGB", (w1_final + w2_final, target_height), (255, 255, 255)
    )
    ...
    mock_status_callback.assert_any_call(
        f"Начинаем обработку изображений из '{input_dir}' в '{output_dir}'..."
    )
    ...
    mock_status_callback.assert_any_call(
        "Создаю разворот: 001_page.png + 002_page.jpg -> 001-002.jpg"
    )
    mock_status_callback.assert_any_call(
        "Копирую последнюю одиночную страницу: 003_single.jpeg -> 003.jpeg"
    )
    mock_status_callback.assert_any_call(
        "Обработка завершена. Обработано/скопировано: 4. Создано разворотов: 1."
    )

    assert mock_progress_callback.call_count == 4
    mock_progress_callback.assert_has_calls(
        [call(0, 4), call(1, 4), call(3, 4), call(4, 4)]
    )

    # 8. Вызовы логгера (примеры)
    mock_logger.info.assert_any_call(
        f"Starting image processing. Input: '{mock_input_path}', Output: '{mock_output_path_obj}'"
    )
    mock_logger.warning.assert_any_call(
        "Skipping file without page number: no_number.jpg"
    )
    ...
    mock_logger.info.assert_any_call(
        "Creating spread: 001_page.png + 002_page.jpg -> 001-002.jpg"
    )
    mock_logger.debug.assert_any_call(
        "    Resizing images to target height: 1210px (using LANCZOS)"
    )
    mock_logger.info.assert_any_call("    Spread created successfully: 001-002.jpg")
    mock_logger.info.assert_any_call(
        "Copying last single page: 003_single.jpeg -> 003.jpeg"
    )
    mock_logger.info.assert_any_call(
        "Processing finished. Processed/copied: 4, Spreads created: 1"
    )
...
def test_process_images_input_not_found(
    ...
    assert result == (0, 0)
    mock_status_callback.assert_any_call(
        f"Ошибка: Папка со страницами '{input_dir}' не найдена."
    )
...
def test_process_images_output_creation_error(
    ...
    assert result == (0, 0)
    mock_status_callback.assert_any_call(
        f"Ошибка создания папки для разворотов '{output_dir}': Permission denied"
    )
    mock_logger.error.assert_called_with(
        f"Ошибка создания папки для разворотов '{output_dir}': Permission denied",
        exc_info=True,
...
def test_process_images_no_numbered_files(
    ...
    mock_status_callback.assert_any_call(
        "В папке не найдено подходящих файлов изображений с номерами."
    )
    mock_logger.warning.assert_called_with(
        f"No processable image files found in {mock_input_path}"
    )
    assert not mock_progress_callback.called
...
def test_process_images_stop_event_triggered(
    ...
    mock_status_callback.assert_any_call(
        "Обработка завершена. Обработано/скопировано: 1. Создано разворотов: 0."
    )

    mock_logger.info.assert_any_call("Processing interrupted by user.")

    mock_progress_callback.assert_has_calls([call(0, 3), call(1, 3)])
...
def test_process_images_copy_error(
    ...
    mock_shutil_copy = mocker.patch(
        "src.image_processing.shutil.copy2", side_effect=shutil.Error("Disk full")
    )
    mocker.patch("src.image_processing.time.sleep")
    ...
    mock_status_callback.assert_any_call(
        "Ошибка при копировании 000_cover.jpg: Disk full"
    )
    mock_logger.error.assert_called_with(
        "Ошибка при копировании 000_cover.jpg: Disk full", exc_info=True
    )
...
def test_process_images_merge_error(
    ...
    mock_utils.get_page_number.side_effect = (
        lambda f: 0 if "000" in f else (1 if "001" in f else (2 if "002" in f else -1))
    )
    mock_utils.is_likely_spread.return_value = False
    ...
    mock_status_callback.assert_any_call(
        "Ошибка при создании разворота для 001_page.png и 002_page.jpg: Cannot save file"
    )
    mock_logger.error.assert_called_with(
        "Ошибка при создании разворота для 001_page.png и 002_page.jpg: Cannot save file",
        exc_info=True,
...
def test_process_images_cover_followed_by_spread(
    ...
    mock_utils.get_page_number.side_effect = (
        lambda f: 0 if "000" in f else (1 if "001" in f else -1)
    )
    ...
    mock_utils.is_likely_spread.assert_called_once_with(
        mock_iterdir_results[1], mock_config.DEFAULT_ASPECT_RATIO_THRESHOLD
    )
    ...
    mock_status_callback.assert_any_call("Копирую обложку: 000_cover.jpg -> 000.jpg")
    mock_status_callback.assert_any_call(
        "Копирую готовый разворот: 001_spread.jpg -> 001.jpg"
    )
    mock_status_callback.assert_any_call(
        "Обработка завершена. Обработано/скопировано: 2. Создано разворотов: 0."
    )

    mock_progress_callback.assert_has_calls([call(0, 2), call(1, 2), call(2, 2)])
...
def test_process_images_single_page_before_spread(
    ...
    mock_status_callback.assert_any_call("Копирую обложку: 000_cover.jpg -> 000.jpg")
    mock_status_callback.assert_any_call(
        "Копирую одиночную страницу (следующий - разворот): 001_single.png -> 001.png"
    )
    mock_status_callback.assert_any_call(
        "Копирую готовый разворот: 002_spread.jpg -> 002.jpg"
    )
    mock_status_callback.assert_any_call(
        "Обработка завершена. Обработано/скопировано: 3. Создано разворотов: 0."
    )

    mock_progress_callback.assert_has_calls(
        [call(0, 3), call(1, 3), call(2, 3), call(3, 3)]
    )
...
def test_process_images_iterdir_os_error(
    ...
    assert result == (0, 0)
    mock_status_callback.assert_any_call(
        f"Ошибка чтения папки '{input_dir}': Permission denied reading directory"
    )
    mock_logger.error.assert_called_with(
        f"Ошибка чтения папки '{input_dir}': Permission denied reading directory",
        exc_info=True,
...
def test_process_images_copy_error_single_before_spread(
    ...
    mock_shutil_copy = mocker.patch(
        "src.image_processing.shutil.copy2", side_effect=copy2_side_effect
    )
    mocker.patch("src.image_processing.time.sleep")
    ...
    mock_status_callback.assert_any_call(
        "Ошибка при копировании одиночной 001_single.png: Copy failed for single before spread"
    )
    mock_logger.error.assert_called_with(
        "Ошибка при копировании одиночной 001_single.png: Copy failed for single before spread",
        exc_info=True,
...
def test_process_images_copy_error_last_single(
    ...
    def copy2_side_effect(src, dst):
        ...
        copy_call_count += 1
        if (
            str(dst) == expected_dest3_str
        ):  # Ошибка при копировании '003_last_single.jpeg'
    ...
    mock_utils.get_page_number.side_effect = (
        lambda f: 0
        if "000" in f
        else (1 if "001" in f else (2 if "002" in f else (3 if "003" in f else -1)))
    )
    mock_utils.is_likely_spread.return_value = False
    ...
    mock_status_callback.assert_any_call(
        "Ошибка при копировании последней одиночной 003_last_single.jpeg: Copy failed for last single"
    )
    mock_logger.error.assert_called_with(
        "Ошибка при копировании последней одиночной 003_last_single.jpeg: Copy failed for last single",
        exc_info=True,
