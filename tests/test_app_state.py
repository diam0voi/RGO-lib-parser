...
def test_set_from_dict(app_state: AppState):
    ...
    assert app_state.pages_dir.get() == config.DEFAULT_PAGES_DIR # Должно остаться значение по умолчанию
...
def test_get_total_pages_int(app_state: AppState, pages_input: str, expected_output: int | None):
...
def test_validate_for_download_invalid(app_state: AppState, field_to_invalidate: str, value: str, expected_error_part: str):
...
def test_validate_for_download_multiple_invalid(app_state: AppState):
    ...
    assert len(errors) == expected_error_count, f"Ожидалось {expected_error_count} ошибок, получено {len(errors)}: {errors}"

    error_messages = set(errors) # Используем set для проверки наличия без учета порядка
    expected_errors = {
        "Базовый URL",
        "ID файла",
        "Имя файла на сайте",
...
def test_validate_for_processing_pages_dir_not_exists(app_state: AppState, tmp_path: Path):
    ...
    assert len(errors) == 1
    assert f"Папка для страниц ('{str(pages_dir)}' не найдена)" in errors[0]
...
def test_validate_for_processing_pages_dir_is_file(app_state: AppState, tmp_path: Path):
    ...
    assert f"Папка для страниц ('{str(pages_file)}' не найдена)" in errors[0]
...
def test_validate_for_processing_permission_error(app_state: AppState, tmp_path: Path):
    ...
    with patch('src.app_state.os.listdir', side_effect=permission_error) as mock_listdir:
